...
(function() {
	    ...
	    xhr.onload = function(e) {
			...
			if(e.currentTarget.status == 200){
				...
				$.each(itemList, function( i, item ) {
					...
					var goodsNodeCopy 	= $('#goodsNode').clone();
					var invenSum		= 0;
					
					goodsNodeCopy.attr('id', '');
					...
					goodsNodeCopy.find('.goodsInventory').html(invenSum);
					
					goodsNodeCopy.find('.nmCilck').data('gid',item.id);
					
					$('#goodsList').append(goodsNodeCopy);
				});
				//페이지를 뿌려주는 append
	...
	yul.page.prototype.clickEvent = function() {
		...
		$(document).on('click', '.nmCilck', function(){
			...
			location.href = "/bgoodsDetail?id=" + $(this).data('gid'); 
		})
